...
```
```{r}
# Extract final fitted workflows from last_fit objects
log_final_model <- extract_workflow(log_final_fit)
tree_final_model <- extract_workflow(tree_final_fit)
rf_final_model <- extract_workflow(rf_final_fit)

# Generate predictions on test set for all three
log_preds_test <- predict(log_final_model, diabetes_test, type = "prob") %>%
  bind_cols(predict(log_final_model, diabetes_test)) %>%
  bind_cols(diabetes_test %>% select(Diabetes_binary))

tree_preds_test <- predict(tree_final_model, diabetes_test, type = "prob") %>%
  bind_cols(predict(tree_final_model, diabetes_test)) %>%
  bind_cols(diabetes_test %>% select(Diabetes_binary))

rf_preds_test <- predict(rf_final_model, diabetes_test, type = "prob") %>%
  bind_cols(predict(rf_final_model, diabetes_test)) %>%
  bind_cols(diabetes_test %>% select(Diabetes_binary))
...
```{r}
evaluate_model <- function(preds) {
  tibble(
    mn_log_loss = mn_log_loss(preds, truth = Diabetes_binary, .pred_Diabetes)$.estimate,
    accuracy = accuracy(preds, truth = Diabetes_binary, estimate = .pred_class)$.estimate
  )
}

results <- bind_rows(
  evaluate_model(log_preds),
  evaluate_model(tree_preds),
  evaluate_model(rf_preds)
) %>%
  mutate(Model = c("Logistic Regression", "Classification Tree", "Random Forest")) %>%
  select(Model, everything())

results %>% arrange(mn_log_loss)

```


Best model is classification Tree
